...
if ($operazione=="salva_modifica") {
	...
	$sql="UPDATE $tb_lotti set DBcodice='$codice', DBprodotto='$prodotto', DBquantita='$quantita', DBprot='$protocollo', marcatura_ce='$marcatura_ce',range_temp='$range_temp',sigla_custom='$sigla_custom',descrizione_custom='$descrizione_custom',gtin1='$gtin1',gtin10='$gtin10',sigla_interna='$sigla',DBmodelli='$DBmodelli',DBmodelli1='$DBmodelli1' $scad $disp WHERE id=$id_lotto";
	$result = $mysqli->query($sql);

	$descr="Data:$datx $ora;Codice:$codice;Descrizione:$prodotto;Scadenza:$data_scad1;Disp:$data_disp1;ID_user:$id_user;Operatore:$operatore;Qta:$quantita;Prot:$protocollo";
...
if ($operazione=="elenco_lotti" || $operazione=="stampa") {
	...
	$cond="1";
	if ($operazione!="stampa") $cond="DBcontrollo<>'!'";
	
	if (strlen($tipo)!=0 && $tipo!="0") $cond.=" and DBtipo='$tipo'";
	if ($tipo_view=="1") $cond.=" and DBverifica<>'S'";
	if ($tipo_view=="2") $cond.=" and DBverifica='S'";
	...
	$sql="SELECT COUNT(DBlotto) as total FROM $tb_lotti WHERE $cond";
	$total_rows=0;
	if ($result = $mysqli->query($sql)) {
		$res = $result->fetch_row();
		$total_rows = $res[0];
	...
	$sql="SELECT id,DBtipo,DBdata,DBprot,DBscadenza,DBlotto,DBcodice,DBprodotto,DBquantita,DBoperatore,DBverifica,DBcontrollo,user_id FROM $tb_lotti WHERE $cond ORDER BY id $ord $limit";
...
if ($operazione=="cercalotto") {
	...
	$sql="SELECT id,DBcontrollo,DBtipo,DBlotto lotto, DBprot, DBcodice codice, DBprodotto prodotto, DBscadenza scadenza, DBdisp disp,DBquantita quantita,DBverifica verifica,DBoperatore,user_id FROM $tb_lotti WHERE $cond order by id desc LIMIT 0,150";
	$result=$mysqli->query($sql);
	$res=array();	
...
if ($operazione=="save_multi") {
	...
	for ($sca=1;$sca<=count($dati);$sca++){
		...
		$qwx="INSERT INTO $tb_lotti(DBtipo,id_lotto, DBlotto, DBcodice, DBprodotto, DBquantita, DBprot, DBcontrollo, DBcq_pf, DBoperatore, DBscadenza, DBdisp, user_id, DBdata, lotto_m, marcatura_ce, range_temp, sigla_custom, descrizione_custom, gtin1, gtin10, sigla_interna, DBmodelli, DBmodelli1) VALUES ('$tipo', $id_lotto, '$lotto', '$codice', '$prodotto', '$quantita', '$prot', '$controllo', '$cq_pf', '$operatore', '$data_scad1', '$data_disp1', $id_user, '$datx', $fl_lm, '$marcatura_ce', '$range_temp', '$sigla_custom','$descrizione_custom', '$gtin1', '$gtin10','$sigla', '$DBmodelli', '$DBmodelli1')";
		...
		$descr="Data:$datx $ora;Codice:$codice $manuale;Descrizione:$prodotto;Scadenza:$data_scad1;Disp:$data_disp1;ID_user:$id_user;Qta:$quantita;Protocollo:$prot";
...
if ($operazione=="load_codici") {
		...
		if ($num>1) $oper="A.COD_ART = '$codice' or A.COD_ART like '$codice-%'";


		$qwx="SELECT A.COD_ART codice,A.DES_ART as descrizione, B.GGSCAD as scadenza, B.GGDISP as disp,B.MARCATURA_CE, B.RANGE_TEMP, B.SIGLA AS SIGLA_CUSTOM, B.DESCRIZIONE AS DESCRIZIONE_CUSTOM from ART_ANA AS A LEFT OUTER JOIN ART_USER AS B on A.COD_ART=B.COD_ART WHERE $oper ORDER BY A.COD_ART";
